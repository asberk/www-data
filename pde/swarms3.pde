...
Swarm swarm2;
color dc1 = color(0);
color dc2 = color(255, 0, 0);
PVector v0, v1;

void setup() {
  size(800, 600, P3D);
  background(255);
  swarm1 = new Swarm(50, 5, dc2);
  swarm2 = new Swarm(50, 5, dc1);
...
void draw() {
  background(255);
camera(width*cos(TWO_PI*mouseX/width), height/2, sin(TWO_PI*mouseX/width)*(height) / tan(PI/6), width/2, height/2, 0, 0, 1, 0);  //  colorMode(RGB);
  //  noStroke();
  //  fill(255, 200);
...
class Dot {
  ...
  Dot() {
    this(new PVector(0,0,0), new PVector(0,0,0), new PVector(0,0,0), 10.0, color(0));
  }
...
class Swarm {
  ...
  final float kappa = 0.3; // scaling coefficient for acceleration
  final float Rc = 100; // for scalingFn
  final float alpha_3 = 0.1;
  final float alpha_4 = 0.5; 
  final float gamma = 0.1;
  final float beta = 0.1;
  final float TOP_SPEED = 2.0;
  ...
  Swarm(int NDots, int NumNbrs, color dc) {
    ...
    for (int j = 0; j < NUM_DOTS; j++) {
      PVector dotpos = new PVector(random(width), random(height), random(-100, 100));
      PVector dotvel = new PVector(random(-1, 1), random(-1, 1), random(-1, 1));
      PVector dotacc = new PVector(random(-1, 1), random(-1, 1), random(-1,1));
      colorMode(HSB);
  ...
  PVector dotAccel(int j, PVector[] nbrs) {
    ...
    PVector rjk = new PVector(0,0,0);
    PVector vel_diff;
    PVector pos_diff;
    PVector drag;
    PVector noisevec = new PVector(randomGaussian(), randomGaussian(), randomGaussian());
